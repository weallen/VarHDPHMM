function [model, loglikHist, llHists] = varAlgo(model, data, init, estep, mstep, varargin)
...
%% Perform Variational algorithm

% sample stick breaking weights
betaWeights = randtruncgem(model.nstates,1.0);
...
while ~done
    [ess, ll] = estep(model, data, betaWeights);
    if verbose
        fprintf('%d\t loglik: %g\n', iter, ll );
    end
    if ~isempty(plotfn)
    ...
    loglikHist(iter) = ll;
    model = mstep(model, ess, betaWeights);

    funObj = @(x) truncgemlikelihood(x,1,model.A);
