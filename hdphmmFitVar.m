function [model, loglikHist] = hdphmmFitVar(data, nstates, type, varargin)
...
model.alpha = 1.0;
model.kappa = 10.0;
model.betaWeights = randtruncgem(model.nstates,1.0);
...
for i=1:model.nstates
    for j=1:model.nstates
        if i == j
            transCounts(i,j) = transCounts(i,j) + model.kappa;
        end
    end
end
...
for i=1:model.nstates
    model.pi(i) = exp(digamma(model.alpha*model.betaWeights(i) + ess.startCounts(i))) / exp(digamma(sum(model.alpha+ess.startCounts)));
end

W = zeros(model.nstates);
for i=1:model.nstates
   for j=1:model.nstates
       %if i==j
       %    ess.transCounts(i,j) = ess.transCounts(i,j) + 1;
       %end
       %W(i,j) = exp(digamma(ess.transCounts(i,j))) / exp(digamma(sum(ess.transCounts(i,:))));
       W(i,j) = exp(digamma(model.alpha*model.betaWeights(i) + ess.transCounts(i,j)) - digamma(model.alpha+sum(ess.transCounts(i,:))));
    end
end    
