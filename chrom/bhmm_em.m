function [LL, prior, transmat, emit1, emit0, nrIterations] = ...
   bhmm_em(data, prior, transmat, emit1, emit0, varargin)
...
[max_iter, thresh, verbose, obs_prior_weight, adj_prior, adj_trans, adj_obs] = ...
   process_options(varargin, 'max_iter', 1000, 'thresh', 1e-4, 'verbose', 1, ...
...
while (num_iter <= max_iter) & ~converged
 % E step
 [loglik, exp_num_trans, exp_num_visits1, exp_num_emit1, exp_num_emit0] = ...
     compute_ess_dhmm(prior, transmat, emit1, emit0, data, A, B);
 ...
 if adj_obs
    for k=1:size(emit1,1)
        for j=1:size(emit1,2)
            total = exp_num_emit1(k,j) + exp_num_emit0(k,j);
            emit1(k,j) = exp_num_emit1(k,j) / total;
            emit0(k,j) = exp_num_emit0(k,j) / total;
        end
    end   
 end
...
function [loglik, exp_num_trans, exp_num_visits1, exp_num_emit1, exp_num_emit0] = ...
   compute_ess_dhmm(startprob, transmat, emit1, emit0, data, A, B)
...
exp_num_visitsT = zeros(K,1);
exp_num_emit1 = A*ones(K,L);
exp_num_emit0 = B*ones(K,L);
loglik = 0;
...
for t=1:T
    currval = logical(obs(t,:));
    exp_num_emit1(:,currval) = exp_num_emit1(:,currval) + repmat(gamma(:,t), [1 sum(currval)]);    
    exp_num_emit0(:,~currval) = exp_num_emit0(:,~currval) + repmat(gamma(:,t), [1 sum(~currval)]);    
  end
end
end
